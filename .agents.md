...
## Argo CD & Apps
- ApplicationSet uses go-template + RollingSync tiers: 1 base system (kube-system, multus, argocd bootstrap), 2 controllers/operators (reloader, metacontroller, external-secrets), 3 PKI/security control-plane (cert-manager, homelab-controller, falco), 4 ingress (envoy-gateway), 5 edge DNS/auth (cloudflared, external-dns, dex), 10 default.
