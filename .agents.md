...
## Argo CD & Apps
- ApplicationSet uses go-template + RollingSync tiers: 1 core k8s, 2 argocd, 3 network (multus), 4 core operators, 5 secondary operators, 6 ingress, 7 edge services, 10 default.
...
- `argocd-secret` is created by the Argo CD chart and mutated by an ExternalSecret; ApplicationSet ignores `metadata.annotations.argocd.argoproj.io/tracking-id` on that Secret to avoid drift.
...
## Observability
- Stack lives in `apps/ops`: VMUI for metrics at `https://metrics.edgard.org` and logs at `https://logs.edgard.org` (both Dex/OIDC protected), VictoriaMetrics single-node for metrics, VictoriaLogs single-node for logs (native + Loki/OTLP endpoints), OpenTelemetry Collector DaemonSet that scrapes `prometheus.io/*` annotated targets and tails pod logs into VictoriaMetrics/VictoriaLogs, kube-state-metrics, and Prometheus Blackbox Exporter for synthetic checks.
