...
- `bootstrap/bootstrap_kind.py` – Kind cluster bring-up and host plumbing (docker context, macvlan attach, kubeconfig patch, busybox preload). Bootstrap expects Docker context `kind-<cluster>` (default `kind-homelab`) to exist.
...
- Bootstrap uses `bootstrap/helmfile.yaml.gotmpl` (run via `task bootstrap:create` after the Kind bring-up) to install pre-Argo dependencies in order: local-path-provisioner (chart + manifests, postsync hook demotes Kind’s default StorageClass), Multus, and Argo CD. Chart refs/versions are read from each app’s `config.yaml`. The Argo release applies `bootstrap/cluster-credentials.sops.yaml` in a presync hook (creates `platform-system` first; uses `SOPS_AGE_KEY_FILE`), then a postsync hook creates the Argo repo Secret from that credential bundle, and another postsync applies the Argo root app. Argo later reconciles the same apps. Paths inside the helmfile are relative to `bootstrap/`.
...
- `task bootstrap:create` creates Kind, attaches workers to a macvlan network, patches kubeconfig endpoint (if remote), then runs helmfile sync (installs Multus/Argo CD, applies cluster secrets via presync, seeds the Argo repo secret, applies the root Argo CD app).
- Busybox warmup remains (kind loads `docker.io/library/busybox:stable` into all nodes).
- Kind uses containerd defaults; we no longer inject Docker Hub credentials or patch registry config during bootstrap.
...
- Warmup pulls/load images into Kind before Argo: busybox (PVC helper jobs) and k8tz webhook (version derived from `apps/kube-system/k8tz/config.yaml`; bootstrap fails if missing/invalid) to avoid first-sync pulls during kube-system wave.
