...
- `kubernetes/argocd/`: Argo CD bootstrap (`root.app.yaml`) and single ApplicationSet (`applications/apps.appset.yaml`) that discovers apps.
- `kubernetes/apps/`: Domain-grouped apps (edge-services, platform-system, ops, media, home-automation, arc, argocd). Each app folder contains `app-config.yaml`, `values.yaml`, and optional `manifests/`.
