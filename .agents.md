...
- Envoy Gateway enforces a global Coraza WAF via `EnvoyExtensionPolicy` on both internal/external gateways using `ghcr.io/corazawaf/coraza-proxy-wasm:0.6.0` with `failOpen: true` (temporary until WASM bootstrap flake is resolved; traffic is allowed if the module cannot be fetched).
