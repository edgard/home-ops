...
## Observability
- Stack lives in `apps/ops`: Grafana UI/alerting, VictoriaMetrics Single as the metrics store (Prom-compatible, datasource uid/name `prometheus` via VM plugin), VictoriaLogs Single for logs (Grafana VM logs plugin), Grafana Alloy as a DaemonSet agent (scrapes cluster metrics + kube-state-metrics and remote-writes to VictoriaMetrics; ships container logs to VictoriaLogs), kube-state-metrics, and Prometheus Blackbox Exporter for lightweight probes. Prometheus and Loki apps are removed in favor of VM/Logs.
