...
- Required keys in `cluster-secrets.sops.yaml` (see the template for exact names):
  ...
  - Dex config (`dex_config_yaml`) with accurate redirect URIs for every Dex-protected HTTPRoute.
...
- Dex (`infra/platform-system/dex`) exposes `id.edgard.org`, pulls config from the `dex-config` ExternalSecret, keeps sqlite data on `/mnt/spool/appdata/dex`, and is referenced directly in Envoy SecurityPolicies—no oauth2-proxy hop in between.
- Every workload ships an internal HTTPRoute (`external-dns.edgard.org/scope: internal`) that targets the `internal` Gateway without auth. External HTTPRoutes target the `external` Gateway, add `external` scope, and attach a `gateway.envoyproxy.io/v1alpha1` `SecurityPolicy` referencing Dex unless the app has a native login flow (currently Atuin and Home Assistant). When you add a new Dex-protected route, remember to add the callback URL to `dex_config_yaml`.
...
- Regenerate Dex bcrypt hashes with `htpasswd -nbB user pass`, update `dex_config_yaml`, add any new redirect URIs, then reapply the secrets bundle and reconcile Flux.
