...
- **flux-system** – Flux Operator + FluxInstance (charts pinned at 0.33.0), the GitRepository (`flux-system`), and the root `cluster-infra`/`cluster-apps` Kustomizations. Bootstrap renders the Helm release once, waits for Ready, and then Flux manages itself.
- **platform-system** – Multus (chart + NAD), External Secrets (operator + ClusterSecretStore pointing at `platform-system/cluster-secrets`), Metrics Server (Insecure TLS + preferred address args, requests removed), Envoy Gateway (chart + config tree), cert-manager (controller + issuer/certificate), External DNS (Cloudflare + UniFi), cloudflared (two replicas, DNSEndpoint), Dex (config from ExternalSecret, sqlite on `/mnt/spool/appdata/dex`), and Reloader.
- **ops** – `gatus` monitoring `status.edgard.org` (sqlite under `/mnt/spool/appdata/gatus`, Telegram alerts) and `kopia` (hostPath mounts for `/mnt/spool/appdata/kopia`, `/mnt/dpool/backup/kopia`, `/mnt/spool/appdata` read-only, privileged init for repo auth via ExternalSecret).
- **home-automation** – Mosquitto (hostPath config/data/logs), Zigbee2MQTT (privileged, `/dev/ttyUSB0`, supplemental group 20, depends on Mosquitto, persists `/mnt/spool/appdata/zigbee2mqtt`), and Home Assistant (Multus IP `192.168.1.246/24`, `/mnt/spool/appdata/home-assistant`, Dex-protected external routes + auth-free internal routes).
- **media** – LinuxServer apps share `PUID=568`, `PGID=568`, `UMASK=022`, mount `/mnt/spool/appdata/<name>` + `/mnt/dpool/media`, and publish dual HTTPRoutes. qbittorrent runs with a Mullvad `gluetun` sidecar using `qbittorrent-gluetun` secrets and `/dev/net/tun`. radarr/sonarr depend on qbittorrent, prowlarr depends on flaresolverr (headless on port 8191), jellyfin takes `192.168.1.245/24`, recyclarr runs supercronic-only, and unpackerr watches `/mnt/dpool/media` with API keys from the `unpackerr` ExternalSecret.
- **web** – `nginx` serves `edgard.org`/`www.edgard.org` from `/mnt/dpool/www` and `echo` provides a Dex-protected `echo.edgard.org` endpoint plus an internal-only route.
- **arc** – `apps/arc/controller` deploys the gha-runner-scale-set-controller. `apps/arc/runners` defines the `arc-homelab` scale set, copies runner externals via an init container, runs a privileged `docker:29-dind` sidecar, mounts `_work`/`dind`, and loads GitHub App credentials from `arc-runners-secret`. The HelmRelease declares a `dependsOn` on the controller.
