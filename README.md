...
- A new `matterbridge` workload in `home-automation` runs `docker.io/luligu/matterbridge:3.3.8`, mounts `/mnt/spool/appdata/matterbridge` as `/root/Matterbridge`, `/mnt/spool/appdata/matterbridge/.matterbridge` as `/root/.matterbridge`, and `/mnt/spool/appdata/matterbridge/certs` as `/root/.mattercert`, and runs `matterbridge -add matterbridge-hass` before launching the daemon with `-frontend 8283 -docker -mdnsinterface net1` so the Home Assistant plugin loads immediately while mDNS stays pinned to the Multus NIC. Keep the plugin config synced manually under `/mnt/spool/appdata/matterbridge/.matterbridge/matterbridge-hass.config.json`.
