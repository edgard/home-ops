...
BOOTSTRAP      ?= $(REPO_ROOT)kind/bootstrap.py
KIND           ?= kind
...
.PHONY: help bootstrap kind-create kind-delete kind-recreate kind-status flux-check flux-status \
    flux-reconcile flux-events flux-image-status flux-alerts render validate require-target \
    sops-list sops-validate sops-edit sops-encrypt sops-decrypt sops-key-generate sops-key-show \
    sops-key-delete bootstrap-destroy

help: ## Show this help
	@awk 'BEGIN {FS = ":.*?## "}; /^[a-zA-Z0-9_\-]+:.*?## / {printf "  \033[36m%-14s\033[0m %s\n", $$1, $$2}' $(MAKEFILE_LIST)
	@echo "\nVariables:"
	@echo "  CLUSTER_NAME=$(CLUSTER_NAME)  KIND_CONFIG=$(KIND_CONFIG)"
	@echo "  KS_NAME=$(KS_NAME)  KS_NAMESPACE=$(KS_NAMESPACE)"
...
bootstrap-destroy: ## Destroy the Kind cluster via the bootstrap script
	python3 "$(BOOTSTRAP)" --destroy
...
flux-reconcile: ## Force reconcile of the primary Git source and kustomization
	$(FLUX) reconcile source git "$(KS_NAME)" --namespace "$(KS_NAMESPACE)"
	$(FLUX) reconcile kustomization "$(KS_NAME)" --namespace "$(KS_NAMESPACE)"

flux-events: ## Show recent Flux events across all namespaces
	$(FLUX) get events -A

flux-image-status: ## Show Flux image repositories and policies
	$(FLUX) get image repository -n $(KS_NAMESPACE)
	$(FLUX) get image policy -n $(KS_NAMESPACE)
