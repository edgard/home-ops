...
- **TrueNAS**: Docker host via SSH. Requires `DOCKER_HOST` env var (e.g., `ssh://user@host`). Paths: `/mnt/spool/appdata`, `/mnt/dpool/media`, `/mnt/dpool/restic`
...
- **Selfhosted**: `changedetection_api_key`, `changedetection_notification_url`, `gatus_telegram_token`, `gatus_telegram_chat_id`, `karakeep_nextauth_secret`, `karakeep_meili_master_key`, `litellm_master_key`, `metamcp_auth_secret`, `paperless_secret_key`, `paperless_admin_user`, `paperless_admin_password`, `paperless_api_token`
...
- **Host Mounts**: Requires `/mnt/spool/appdata`, `/mnt/dpool/media`, `/mnt/dpool/restic` on TrueNAS host
...
- **Selfhosted**: atuin, changedetection, gatus, homepage, karakeep, litellm, metamcp, paperless
